...
const $7b075eedcc8429e2$var$loadTensorFlow = async ()=>{
...
class $7b075eedcc8429e2$var$TensorFlowAnalyzer {
    constructor(){
        this.model = null;
    ...
    async initialize() {
        ...
        if (this.isLoading) {
            ...
            while(this.isLoading)await new Promise((resolve)=>setTimeout(resolve, 100));
            return this.isReady;
        }
        this.isLoading = true;
        ...
        try {
            ...
            await $7b075eedcc8429e2$var$loadTensorFlow();
            ...
            if (error.message === 'TIMEOUT') console.log("[SafeGuard TensorFlow] \u26A0\uFE0F  CDN loading timed out (normal if offline or CSP blocked)");
            else console.log("[SafeGuard TensorFlow] \u26A0\uFE0F  Failed to load:", error.message);
    ...
    async analyzeText(text) {
        if (!this.isReady) {
            ...
            if (!this.initAttempted) {
                ...
                if (!success) return {
                    ...
                    level: 0,
                    detectedTypes: [],
                    source: 'tensorflow-unavailable'
                };
            } else // Already attempted initialization and failed - skip silently
            return {
                isToxic: false,
                level: 0,
                detectedTypes: [],
        ...
        try {
            ...
            return {
                ...
                level: 0,
                detectedTypes: [],
                source: 'tensorflow-error'
            };
        }
    }
}
// Initialize TensorFlow analyzer
const $7b075eedcc8429e2$var$tensorflowAnalyzer = new $7b075eedcc8429e2$var$TensorFlowAnalyzer();
class $7b075eedcc8429e2$var$ConversationMonitor {
    constructor(){
        this.platform = null;
    ...
    async detectPlatform() {
        try {
            ...
            for (const [key, platformData] of Object.entries(config.platforms)){
                const domains = Array.isArray(platformData.domain) ? platformData.domain : [
                    platformData.domain
                ];
                ...
                inputs.forEach((input)=>{
                    if (!this.observedElements.has(input)) {
                        ...
                        input.addEventListener('keydown', (e)=>{
                            if (e.key === 'Enter' && !e.shiftKey) setTimeout(()=>this.handleMessageSent(input), 100);
                        });
                    }
                });
            });
        ...
        const observer = new MutationObserver((mutations)=>{
            mutations.forEach((mutation)=>{
                mutation.addedNodes.forEach((node)=>{
                    if (node.nodeType === Node.ELEMENT_NODE) // Check if this is a message
                    selectors.forEach((selector)=>{
                        if (node.matches && node.matches(selector)) this.handleNewMessage(node);
                        // Check children
    ...
    monitorImages() {
        ...
        const isMessageImage = (img)=>{
            ...
            const hasMessageImageClass = img.classList.contains('xz74otr') || img.classList.contains('x193iq5w') || img.classList.contains('xmz0i5r');
        ...
        selectors.forEach((selector)=>{
            ...
            existingImages.forEach((img)=>{
                if (!isProfilePicture(img) && isMessageImage(img)) {
                    console.log(`[SafeGuard Kids] \u{2713} Message image detected: ${img.width}x${img.height}`);
                    this.handleImage(img);
                } else console.log(`[SafeGuard Kids] \u{2717} Skipping (profile pic or UI element): ${img.width}x${img.height}`);
            });
        });
        ...
        messageContainers.forEach((container)=>{
            ...
            allImages.forEach((img)=>{
                if (!isProfilePicture(img) && isMessageImage(img)) {
                    console.log(`[SafeGuard Kids] \u{2713} Fallback message image: ${img.width}x${img.height}`);
                    this.handleImage(img);
                }
            });
        });
                ...
                mutation.addedNodes.forEach((node)=>{
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        ...
                        selectors.forEach((selector)=>{
                            if (node.matches && node.matches(selector)) {
                                ...
                                if (node.complete) {
                                    if (!isProfilePicture(node) && isMessageImage(node)) {
                                        console.log(`[SafeGuard Kids] \u{2713} New message image: ${node.width}x${node.height}`);
                                        this.handleImage(node);
                                    } else console.log(`[SafeGuard Kids] \u{2717} Skipping new image (profile): ${node.width}x${node.height}`);
                                } else // Image not loaded yet, wait for load event
                                node.addEventListener('load', ()=>{
                                    if (!isProfilePicture(node) && isMessageImage(node)) {
                                        console.log(`[SafeGuard Kids] \u{2713} New message image (after load): ${node.width}x${node.height}`);
                                        this.handleImage(node);
                                    }
                                }, {
                                    once: true
                            ...
                            if (images.length > 0) console.log(`[SafeGuard Kids] Found ${images.length} new child images`);
                            images.forEach((img)=>{
                                if (img.complete) {
                                    if (!isProfilePicture(img) && isMessageImage(img)) {
                                        console.log(`[SafeGuard Kids] \u{2713} New child message image: ${img.width}x${img.height}`);
                                        this.handleImage(img);
                                    }
                                } else img.addEventListener('load', ()=>{
                                    if (!isProfilePicture(img) && isMessageImage(img)) {
                                        console.log(`[SafeGuard Kids] \u{2713} New child message image (after load): ${img.width}x${img.height}`);
                                        this.handleImage(img);
                                    }
                                }, {
                                    once: true
                        ...
                        if (node.tagName === 'IMG' || node.querySelector) {
                            const allNewImages = node.tagName === 'IMG' ? [
                                node
    ...
    handleMessageSent(inputElement) {
        const text = inputElement.textContent || inputElement.value;
        if (text && text.trim()) // Service worker will store this in conversation memory
        this.analyzeText(text, 'sent');
    }
    handleNewMessage(messageElement) {
        const messageId = this.getMessageId(messageElement);
        if (this.lastAnalyzedMessages.has(messageId)) return;
    ...
    handleImage(imageElement) {
        ...
        if (!src || src.startsWith('data:image/svg') || imageElement.width < 50 || imageElement.height < 50) return;
        ...
        if (this.analyzedImages && this.analyzedImages.has(imageId)) return;
        if (!this.analyzedImages) this.analyzedImages = new Set();
        ...
        while(element && depth < 10){
            ...
            for (const selector of sentSelectors)if (element.matches && element.matches(selector)) {
                imageType = 'sent';
                break;
            }
    ...
    async analyzeText(text, type, element = null) {
        ...
        console.log('');
        console.log("\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550");
        if (type === 'sent') console.log("\uD83D\uDCE4 SENT MESSAGE");
        ...
        try {
            tensorflowResult = await $7b075eedcc8429e2$var$tensorflowAnalyzer.analyzeText(text);
        } catch (error) {
        ...
        console.log(`\u{1F4E8} Sending ${messageTypeLabel} message to SafeGuard for analysis...`);
        try {
            ...
            console.log(`\u{2705} SafeGuard Analysis Results for ${messageTypeLabel} MESSAGE:`, response);
            console.log('   Text analyzed:', text);
            console.log('   Threat level:', response.threat ? response.threat.level : 0);
            console.log("\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550");
            console.log('');
            if (response && response.threat) this.handleThreat(response.threat, text, type, element);
        } catch (error) {
    ...
    async analyzeImage(imageElement, imageType) {
        ...
        console.log('');
        console.log("\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550");
        if (imageType === 'sent') console.log("\uD83D\uDCE4 SENT IMAGE");
        else console.log("\uD83D\uDCE5 RECEIVED IMAGE");
        console.log("\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550");
        ...
        try {
            ...
            if (response && response.threat) {
                const threatDescription = imageType === 'sent' ? `Child sent inappropriate image` : `Child received inappropriate image`;
    ...
    handleThreat(threat, content, type, element) {
        ...
        if ((this.mode === 'active' || this.mode === 'both') && threat.level >= 7) this.showWarningPopup(threat);
        // Passive mode or Both mode: Notify parent for critical threats
        if ((this.mode === 'passive' || this.mode === 'both') && (threat.level >= 9 || threat.critical)) chrome.runtime.sendMessage({
            action: 'notifyParent',
            data: {
                threat: threat,
                content: content,
        ...
        if (this.mode === 'both' && threat.level >= 7 && threat.level < 9) chrome.runtime.sendMessage({
            action: 'notifyParent',
            data: {
                threat: threat,
                content: content,
    ...
    showWarningPopup(threat) {
      ...
      <div class="safeguard-warning-modal">
        <div class="safeguard-warning-icon">\u{26A0}\u{FE0F}</div>
    ...
    getWarningMessage(threat) {
        ...
        let explanation = '';
        if (threat.explanation) // Extract key reason from AI explanation
        ...
        if (threat.level >= 9) return `
        <strong style="color: #dc2626; font-size: 18px;">\u{26A0}\u{FE0F} DANGER</strong>
      ...
      `;
        else if (threat.level >= 7) return `
        <strong style="color: #ea580c; font-size: 16px;">\u{26A0}\u{FE0F} Warning</strong>
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', ()=>{
    new $7b075eedcc8429e2$var$ConversationMonitor();
});
else new $7b075eedcc8429e2$var$ConversationMonitor();
